...
- `Inference`: The next high-value unit is one tightly bounded implementation slice that executes the planned story-split proof without broadening into hero, media-system, or composition-heavy behavior.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-052` implementation scope until the planned story-split slice is executed and verified.
- `Inference`: Any newly discovered need for protected-surface edits, `project/blocks/`, JS-driven behavior, a second section, a second template, video behavior, gallery behavior, overlay treatment, hero semantics, or page-composition behavior remains a replanning trigger rather than permission to expand the next unit.

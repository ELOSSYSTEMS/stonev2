...
- `Inference`: The next high-value unit is one tightly bounded implementation slice that executes the planned trust-stat-strip proof without broadening into guarantees, counters, or ratings behavior.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-049` implementation scope until the planned trust-stat-strip slice is executed and verified.
- `Inference`: Any newly discovered need for protected-surface edits, `project/blocks/`, JS-driven behavior, a second section, a second template, counter animation, chart behavior, badges, icons, guarantees, ratings, or page-composition behavior remains a replanning trigger rather than permission to expand the next unit.

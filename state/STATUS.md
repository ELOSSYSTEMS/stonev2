...
- `Inference`: The next high-value unit is one tightly bounded documentation-only planning slice that defines the next minimal runtime proof after verified minimal trust FAQ completion.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-036` planning scope until the next bounded slice is explicitly planned and approved.

...
- `Inference`: The next high-value unit is one tightly bounded product-aware implementation slice under that new boundary.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-076` implementation scope under the new PDP-companion boundary.
- `Inference`: Any newly discovered need for protected-surface edits, `project/blocks/`, JS-driven behavior, a second section, a second template, broader object behavior, product-card behavior, slider behavior, drawer behavior, overlay behavior, archive/discovery behavior, or page-composition behavior remains a failure signal rather than permission to broaden the first PDP-aware slice.

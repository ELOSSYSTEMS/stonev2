...
- `Inference`: The next high-value unit is one tightly bounded implementation slice that proves utility layout composition without broadening beyond the approved five runtime files.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-013` five-file implementation scope for minimal utility layout composition.

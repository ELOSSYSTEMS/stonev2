...
- `Inference`: The next high-value unit is one tightly bounded implementation slice that executes the planned process-steps proof without broadening into accordion, campaign, or composition-heavy behavior.
...
- `Verified`: Broader runtime build-out remains blocked beyond the approved `UNIT-058` implementation scope until the planned process-steps slice is executed and verified.

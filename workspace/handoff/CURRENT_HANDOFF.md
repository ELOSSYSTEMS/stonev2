...
- `Verified`: `state/NEXT_ACTION.md` now points to `UNIT-052`, the bounded implementation unit for that planned story-split slice.
...
- `Inference`: `UNIT-052` should implement exactly one bounded story split slice and must fail closed if the proof starts depending on hero, video, gallery, or shoppable-media behavior.

## Recommended Next Step

- `Verified`: Execute `UNIT-052` from `state/NEXT_ACTION.md` and implement exactly one bounded story split slice within the approved five runtime files.
...
- `Verified`: Use the verified runtime chain through `UNIT-051` as the starting boundary proof for the next implementation unit.
- `Verified`: Keep the next unit inside `project/assets/stone-base.css`, `project/sections/stone-story-split.liquid`, `project/templates/page.stone-runtime-story-split.json`, `project/locales/he.default.schema.json`, `project/locales/en.schema.json`, plus `state/` and `workspace/handoff/CURRENT_HANDOFF.md` only.

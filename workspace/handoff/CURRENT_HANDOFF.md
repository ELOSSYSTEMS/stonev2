...
- `Verified`: `state/NEXT_ACTION.md` now points to `UNIT-058`, the bounded implementation unit for that planned process-steps slice.
...
- `Inference`: `UNIT-058` should implement exactly one bounded process-steps slice and must fail closed if the proof starts depending on accordions, tabs, icons, or campaign framing.

## Recommended Next Step

- `Verified`: Execute `UNIT-058` from `state/NEXT_ACTION.md` and implement exactly one bounded process-steps slice within the approved five runtime files.
...
- `Verified`: Use the verified runtime chain through `UNIT-057` as the starting boundary proof for the next implementation unit.
- `Verified`: Keep the next unit inside `project/assets/stone-base.css`, `project/sections/stone-process-steps.liquid`, `project/templates/page.stone-runtime-process-steps.json`, `project/locales/he.default.schema.json`, `project/locales/en.schema.json`, plus `state/` and `workspace/handoff/CURRENT_HANDOFF.md` only.

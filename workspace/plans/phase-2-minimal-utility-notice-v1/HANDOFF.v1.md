# Phase 2 Minimal Utility Notice Handoff v1

## Intent

- `Verified`: The next separately governed runtime unit should implement one bounded minimal utility notice slice.
...
- `Inference`: Add neutral CSS primitives for a restrained contained notice surface.
- `Inference`: Add one utility-only notice section that reuses `stone-section-shell` unchanged.
...
- `Verified`: Do not add actions, icons, severity states, announcement behavior, support behavior, trust framing, promo framing, or campaign framing.

## Verification Intent

- `Verified`: Confirm the runtime diff stays inside the approved five-file boundary.
- `Verified`: Confirm the section remains utility-only and distinct from open richtext through contained notice treatment only.

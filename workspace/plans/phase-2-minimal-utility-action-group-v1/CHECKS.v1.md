# Phase 2 Minimal Utility Action Group Checks v1

## Acceptance Checks

| ID | Requirement | Evidence Needed |
| --- | --- | --- |
| AC-01 | Only the approved runtime file set is created or modified under `project/`. | Diff shows changes only in `project/assets/stone-base.css`, `project/sections/stone-utility-action-group.liquid`, `project/templates/page.stone-runtime-action-group.json`, `project/locales/he.default.schema.json`, and `project/locales/en.schema.json`. |
| AC-02 | The slice proves one utility-only native action-group behavior. | File review shows `project/sections/stone-utility-action-group.liquid` renders a bounded grouped set of native links or buttons without hero, trust, promotional, editorial, or commerce semantics. |
...
| AC-04 | The template host stays neutral and isolated. | `project/templates/page.stone-runtime-action-group.json` mounts only the utility action-group validation section and does not introduce homepage ordering, preset defaults, or multi-section sequencing. |
| AC-05 | The base-layer extension stays substrate-level. | File review of `project/assets/stone-base.css` shows neutral action-group primitives only, without family-specific styling, media-led behavior, storefront polish, or protected-surface backtracking. |
...
| RC-04 | Interaction boundary | Confirm the section stays non-interactive beyond native links or buttons and does not grow into JS-driven behavior. | Drawer, modal, toggle, accordion, popover, or runtime-JS dependence appears. |
| RC-05 | Template scope | Confirm no homepage, collection, PDP, preset, or additional alternate-template composition is introduced. | Any template file beyond `project/templates/page.stone-runtime-action-group.json` or any multi-section sequencing. |
...
- `Verified`: Review `project/sections/stone-utility-action-group.liquid` and reject any product, collection, article, blog, hero, story, trust, merchandising, editorial, or media-led behavior.
- `Verified`: Review `project/sections/stone-utility-action-group.liquid` and reject any need for runtime JS, protected-surface edits, `project/blocks/`, or broader interaction systems.
- `Verified`: Review `project/templates/page.stone-runtime-action-group.json` and reject any extra section composition or preset-like sequencing.

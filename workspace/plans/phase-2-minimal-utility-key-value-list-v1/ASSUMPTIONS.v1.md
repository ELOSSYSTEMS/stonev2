# Phase 2 Minimal Utility Key-Value List Assumptions v1

## Scope Assumptions

| ID | Type | Statement | Basis | Impact If Wrong | Status |
| --- | --- | --- | --- | --- | --- |
| A-01 | `Verified` | The verified shared section shell and verified neutral page-host pattern are sufficient to host one utility-only key-value list section without editing `project/snippets/stone-section-shell.liquid`, `project/layout/theme.liquid`, `project/assets/stone-runtime.js`, or `project/config/settings_schema.json`. | `workspace/plans/phase-2-minimal-section-runtime-substrate-v1/PLAN.v1.md`; review of the verified runtime files under `project/`. | The slice is not actually bounded to the approved five runtime files and must be replanned. | open |
| A-02 | `Inference` | Extending `project/assets/stone-base.css` is enough to support a neutral key-value list surface, so no additional CSS asset is needed yet. | `project/layout/theme.liquid` already loads `stone-base.css` globally and prior bounded slices used it as the shared substrate surface. | The slice expands into extra asset files and breaks the approved runtime boundary. | open |
| A-03 | `Inference` | One new utility-only section is enough to validate structured term-value presentation without requiring homepage sequencing or broader page composition. | The runtime already has a verified neutral page-host pattern and does not yet need multi-section composition to prove one additional utility-family behavior. | The unit grows into multiple sections or page-level composition and requires replanning. | open |
| A-04 | `Inference` | The key-value proof can stay bounded to native text content only, with no JS-driven reveal, comparison toggle, or accordion behavior. | The planning intent explicitly limits the future slice to non-interactive structured information presentation and rejects JS-driven behavior. | The slice expands into interaction systems and invalidates the plan. | open |
| A-05 | `Inference` | A narrow key-value surface can stay neutral and utility-only without becoming a PDP specification module, trust facts panel, service matrix, or promotional explainer. | The utility and layout support family in `STONEV2_SECTION_FAMILY_LEDGER.md` explicitly permits structural helpers and utility prompts while rejecting broader family semantics. | The future section broadens into a different family and requires replanning. | open |
...
- `Verified`: The slice stays minimal only if it adds one new utility-only key-value list section, one new neutral page template, one bounded base-CSS extension, and paired locale updates.
...
- `Verified`: The future slice must remain non-interactive and must not require runtime JS.
...
- `Verified`: Replan if execution needs `project/blocks/`, a new snippet, a second new section, a second new template, JS-driven behavior, media handling, or comparison-table behavior.

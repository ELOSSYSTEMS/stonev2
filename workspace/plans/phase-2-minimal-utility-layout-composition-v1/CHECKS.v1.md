# Phase 2 Minimal Utility Layout Composition Checks v1

## Acceptance Checks

| ID | Requirement | Evidence Needed |
| --- | --- | --- |
| AC-01 | Only the approved runtime file set is created or modified under `project/`. | Diff shows changes only in `project/assets/stone-base.css`, `project/sections/stone-utility-layout-composition.liquid`, `project/templates/page.stone-runtime-layout-composition.json`, `project/locales/he.default.schema.json`, and `project/locales/en.schema.json`. |
| AC-02 | The slice proves one utility-only layout-composition behavior beyond simple vertical block stacking. | File review shows `project/sections/stone-utility-layout-composition.liquid` partitions a narrow content-block subset into exactly two named utility regions and renders them through a responsive grouped or columnar layout. |
...
| AC-04 | The template host stays neutral and isolated. | `project/templates/page.stone-runtime-layout-composition.json` mounts only the utility layout-composition validation section and does not introduce homepage ordering, preset defaults, or multi-section sequencing. |
| AC-05 | The base-layer extension stays substrate-level. | File review of `project/assets/stone-base.css` shows neutral utility composition primitives only, without family-specific styling, storefront polish, or protected-surface backtracking. |
...
| AC-07 | No global settings, runtime JS behavior, snippet files, block files, second section files, or second template files enter the slice. | Diff shows no edits to `project/config/settings_schema.json`, `project/assets/stone-runtime.js`, `project/snippets/`, or `project/blocks/`, and no extra section or template file appears. |
...
| RC-04 | Composition boundary | Confirm the section stays bounded to exactly two utility regions and does not grow into nested composition or multi-section layout. | More than two regions, nested grouping structures, or multi-section template sequencing. |
| RC-05 | Template scope | Confirm no homepage, collection, PDP, preset, or additional alternate-template composition is introduced. | Any template file beyond `project/templates/page.stone-runtime-layout-composition.json` or any multi-section sequencing. |
...
- `Verified`: Review `project/sections/stone-utility-layout-composition.liquid` and reject any product, collection, article, blog, hero, story, trust, merchandising, or editorial behavior.
- `Verified`: Review `project/sections/stone-utility-layout-composition.liquid` and reject any need for more than two utility regions, nested grouping, or protected-surface edits.
- `Verified`: Review `project/templates/page.stone-runtime-layout-composition.json` and reject any extra section composition or preset-like sequencing.
- `Verified`: Review `project/assets/stone-base.css` and reject any shell rewrite, protected-surface coupling, or family-level styling.
